...
$DOC_TITLE = "G&eacute;rer les mobiliers";
...
switch (vb($_REQUEST['mode'])) {
	...
	case "ajout" :
		affiche_formulaire_ajout_mobiliers(vn($_REQUEST['categorie_id']));
		break;

	case "modif" :
		affiche_formulaire_modif_mobiliers(vn($_REQUEST['id']));
		break;
		
	

	case "suppr" :
		supprime_mobiliers(vn($_REQUEST['id']));
		affiche_liste_mobiliers($start, $_POST);
		break;
	...
	case "insere" :
		...
		$message_erreur = valide_form_mobiliers($frm, $erreurs);
		...
		insere_mobiliers(vn($_REQUEST['id']), vb($img1), vb($img2), vb($img3), vb($img4),vb($img5), vb($img6), vb($img7), vb($img8), $_POST);
	
		affiche_liste_mobiliers($start, 0);
		...
		affiche_formulaire_ajout_mobiliers(vn($_REQUEST['id']));
	...
	case "maj" :
		...
		maj_mobiliers($frm['id'], vb($img1), vb($img2), vb($img3), vb($img4),  vb($img5), vb($img6), vb($img7), vb($img8), $_POST);
		...
		affiche_formulaire_modif_mobiliers($frm['id']);
...
function affiche_arbo_cat(&$sortie, &$selectionne, $parent=0, $indent="") {
	...
	$sql = "(SELECT id,  nom, description, image, etat, position, nb FROM categories WHERE parent_id = $parent ORDER BY position" or DIE('Une erreur de connexion &agrave; la base s est mobilierse ' . __LINE__ . '.<p>' . MYSQL_ERROR());
    
function affiche_formulaire_ajout_mobiliers($categorie_id = 0) {
/* Affiche un formulaire vierge pour ajouter un mobiliers */
	...
	affiche_formulaire_mobiliersx();
}

function affiche_formulaire_modif_mobiliers($id) {
  /* Affiche le formulaire de modification pour l'mobiliers s&eacute;lectionn&eacute; */

	global $frm,$categorie_options;

	/* Charge les informations du mobiliers */
	$qid = query("
	SELECT *
	FROM mobiliers
	WHERE id = $id
	...
	/* Charge les cat&eacute;gories du mobiliers */
	$qid = query("
	SELECT categorie_id
	FROM mobiliers_categories
	WHERE mobiliers_id = $id
	");
	...
	affiche_formulaire_mobiliers();
}



function affiche_formulaire_mobiliers() {
	GLOBAL $frm;
	GLOBAL $id;
...
<tr>
	<td width="50%">
		...
		<?=$categorie_options?>
				...
				<tr>
					<td class="normal">Etat du mobiliers :</td>
					<td>
				...
				<tr>
					<td class="label" colspan=2><font color="red">Nom du mobiliers *</font>:
					<?php
				...
				<tr>
					<td class="entete" colspan="2">Photos des mobiliers :</td>
				</tr>
...
<?php
				...
				<tr valign=top>
					<td class="label" colspan="2">Description (affichage dans les fiches mobiliers) :<br /></td>
				</tr>
				<tr valign=top>
					<td class=normal colspan=2>
					<p>
...
function supprime_mobiliers($id) {
/* Supprime l mobiliers sp&eacute;cifici&eacute; par $id. Il faut supprimer l mobiliers
 * puis les entr&eacute;es correspondantes de la table mobiliers_categories. */

	/* Charge les infos du mobiliers. */
	$sql = query("
	...
	$prod = fetch_assoc($qid);
		...
		/* Efface l mobiliers */
	query("DELETE FROM mobiliers WHERE id = $id");

	/* Efface ce mobiliers de la table mobiliers_categories */
	
	$sqlCat = "SELECT categorie_id FROM mobiliers_categories WHERE mobiliers_id = $id";
	...
	if ($resCat->RowCount() > 0){
		...
		while ($cat = fetch_assoc($resCat)) {
			...
			or DIE('Une erreur de connexion &agrave; la base s est mobilierse ' . __LINE__ . '.<p>' . MYSQL_ERROR());
		
		}
		
	}
	
	query("DELETE FROM mobiliers_categories WHERE mobiliers_id = $id");
	
	
	
	?>
		<p class="normal">
		L mobiliers <b><?echo html_entity_decode($prod['nom']); ?></b> a &eacute;t&eacute; effac&eacute;.
		</p>
	<?php
}

function supprime_fichier($id, $file) {
/* Supprime le mobiliers sp&eacute;cifici&eacute; par $id. Il faut supprimer le mobiliers
 * puis les entr&eacute;es correspondantes de la table mobiliers_categories. */
	global $the_path ;
	/* Charge les infos du mobiliersx. */

	switch($file) {

	case "image1" :
	$sql = "SELECT image1 FROM mobiliers WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	query("UPDATE mobiliers SET image1 = '' WHERE id = '$id'");
	break;
	
	case "image2" :
	$sql = "SELECT image2 FROM mobiliers WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE mobiliers SET image2 = '' WHERE id = '$id'");
	break;
	
	case "image3" :
	$sql = "SELECT image3 FROM mobiliers WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE mobiliers SET image3 = '' WHERE id = '$id'");
	break;
	
	case "image4" :
	$sql = "SELECT image4 FROM mobiliers WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE mobiliers SET image4 = '' WHERE id = '$id'");
	break;
	
    case "image5" :
	$sql = "SELECT image5 FROM mobiliers WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE mobiliers SET image5 = '' WHERE id = '$id'");
	break;
	
	case "image6" :
	$sql = "SELECT image6 FROM mobiliers WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE mobiliers SET image6 = '' WHERE id = '$id'");
	break;
	
	case "image7" :
	$sql = "SELECT image7 FROM mobiliers WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE mobiliers SET image7 = '' WHERE id = '$id'");
	break;

    case "image8" :
	$sql = "SELECT image8 FROM mobiliers WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE mobiliers SET image8 = '' WHERE id = '$id'");
	break;
...
function insere_mobiliers($id, $img1, $img2, $img3, $img4, $img5, $frm) {
/* Ajoute un nouveau sous-mobiliers sous le parent $id.  Les champs sont dans la variable $frm */
	...
	$mobiliers = addSlashes($frm['mobiliers']);
	$type_mobiliers = addSlashes($frm['mobiliers']);
	$materiaux = addSlashes($frm['materiaux']);
	/*ajoute l mobiliers dans la table mobiliers */
	...
	$qid =query($sqlProd) or DIE('Une erreur de connexion &agrave; la base s est mobilierse ' . __LINE__ . '.<p>' . MYSQL_ERROR());
	
	/* r&eacute;cup&egrave;re l'id du mobiliers cr&eacute;&eacute; */
	$mobiliers_id = mysql_insert_id();

	/* ajoute le mobiliers sous les cat&eacute;gories sp&eacute;cifi&eacute;es */
	for ($i = 0; $i < count(vn($frm['categories'])); $i++) {
		$qid = query("
		INSERT INTO mobiliers_categories (categorie_id, mobiliers_id)
		VALUES ('{$frm['categories'][$i]}', '$mobiliers_id')
		");
...
function maj_mobiliers($id, $img1, $img2, $img3, $img4, $img5, $img6, $img7, $img8,  $frm) {
/* Met &agrave; jour le mobiliers $id avec de nouvelles valeurs. Les champs sont dans $frm */

	valide($frm['on_mobiliers']);
	
	/* Met &agrave; jour la table mobiliers */
	
	$sql = "";
	$sql .= "
	UPDATE mobiliers SET
	...
	$sql .= "
		...
		,etat = '$frm[etat]'
		,date_maj = now()
		,on_mobiliers= '$frm[onmobiliers]'
		
		
	WHERE id = ".intval($id)."
	";
	
	$qid = query($sql) or DIE('Une erreur de connexion &agrave; la base s est mobilierse ' . __LINE__ . '.<p>' . MYSQL_ERROR());
	...
	/* Efface toutes les cat&eacute;gories auxquelles le mobiliers est associ&eacute; */
	$qid = query("
	DELETE FROM mobiliers_categories
	WHERE mobiliers_id = $id
	");
	

	/* Ajoute les  associations pour toutes les cat&eacute;gories auxquelles ce mobiliers
...
function affiche_liste_mobiliers($start, $frm) 
{
global $wwwroot;
global $categorie_options;
global $repertoire_images;
...
<table border="0" cellpadding="0" cellspacing="2" width="100%">
    ...
    <tr>
      <td class="normal">Etat du mobiliers : <br />
	  <span class="normal">
...
<tr>
	<td class="entete" colspan="10">Liste des mobiliers</td>
</tr>
<tr><td colspan="10" class="normal">
<img src="images/add.png" width="16" height="16" alt="" border="0" align="middle"><a href="<?echo $wwwroot?>/gestions/mobiliers.php?mode=ajout">Ajouter un mobiliers</a>
</td></tr>
			...
			if (isset($frm['categorie'])) {
				...
				$Links-> SqlRequest = "SELECT p.id, p.nom, p.description, p.image1, p.etat, p.date_maj FROM mobiliers p, mobiliers_categories pc,categories c WHERE p.id = pc.mobiliers_id AND c.id = pc.categorie_id ".$where." ORDER BY parent_id ASC";	
				
			} else {
			
				$Links-> SqlRequest = "SELECT p.id, p.diametre, p.capacite, p.formes, p.type_paroi, p.materiaux,  p.nom_".$_SESSION['langue'].", p.description_".$_SESSION['langue'].", p.image1, p.pdf, p.etat, p.date_maj FROM mobiliers p ORDER BY id DESC";
			...
			echo "<tr><td class=\"normal\"><b>Aucun mobiliers enregistr&eacute; dans la base pour ce crit&egrave;re</b></td></tr>"; } 
			...
			?>
					<tr bgcolor="<?echo ($i % 2 == 0 ? '#F4F4F4' : '#ffffff' );?>">
						...
						<a onClick="Javascript:return confirm('&ecirc;tes-vous s&ucirc;r de vouloir supprimer le mobiliers <?=$ligne['nom_'.$_SESSION['langue'].''] ?> ?');" class=normal title="Supprimer <?=$ligne['nom_'.$_SESSION['langue'].''] ?>" href="<?=$_SERVER['PHP_SELF']?>?mode=suppr&id=<?=$ligne['id'] ?>">
...
?>
						...
						<td class=normal align="center"><a class=normal title="Modifier ce mobiliers" href="<?=$_SERVER['PHP_SELF']?>?mode=modif&id=<?=$ligne['id'] ?>"><?=html_entity_decode($ligne['nom_'.$_SESSION['langue'].''])?></a></td>
...
<?PHP
function valide_form_mobiliers(&$frm, &$erreurs) {
