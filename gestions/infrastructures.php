...
$DOC_TITLE = "G&eacute;rer les infrastructures";
...
switch (vb($_REQUEST['mode'])) {
	...
	case "ajout" :
		affiche_formulaire_ajout_infrastructures(vn($_REQUEST['categorie_id']));
		break;

	case "modif" :
		affiche_formulaire_modif_infrastructures(vn($_REQUEST['id']));
		break;
		
	

	case "suppr" :
		supprime_infrastructures(vn($_REQUEST['id']));
		affiche_liste_infrastructures($start, $_POST);
		break;
	...
	case "insere" :
		...
		$message_erreur = valide_form_infrastructures($frm, $erreurs);
		...
		insere_infrastructures(vn($_REQUEST['id']), vb($img1), vb($img2), vb($img3), vb($img4),vb($img5), vb($img6), vb($img7), vb($img8), $_POST);
	
		affiche_liste_infrastructures($start, 0);
		...
		affiche_formulaire_ajout_infrastructures(vn($_REQUEST['id']));
	...
	case "maj" :
		...
		maj_infrastructures($frm['id'], vb($img1), vb($img2), vb($img3), vb($img4),  vb($img5), vb($img6), vb($img7), vb($img8), $_POST);
		...
		affiche_formulaire_modif_infrastructures($frm['id']);
...
function affiche_arbo_cat(&$sortie, &$selectionne, $parent=0, $indent="") {
	...
	$sql = "(SELECT id,  nom, description, image, etat, position, nb FROM categories WHERE parent_id = $parent ORDER BY position" or DIE('Une erreur de connexion &agrave; la base s est infrastructurese ' . __LINE__ . '.<p>' . MYSQL_ERROR());
    
function affiche_formulaire_ajout_infrastructures($categorie_id = 0) {
/* Affiche un formulaire vierge pour ajouter un infrastructures */
	...
	$frm['nom'] = "Nom de infrastructures";
	$frm['descriptif'] = "";
	$frm['description'] = "";
	...
	affiche_formulaire_infrastructuresx();
}

function affiche_formulaire_modif_infrastructures($id) {
  /* Affiche le formulaire de modification pour l'infrastructures s&eacute;lectionn&eacute; */

	global $frm,$categorie_options;

	/* Charge les informations du infrastructures */
	$qid = query("
	SELECT *
	...
	/* Charge les cat&eacute;gories du infrastructures */
	$qid = query("
	SELECT categorie_id
	FROM infrastructures_categories
	WHERE infrastructures_id = $id
	");
	...
	affiche_formulaire_infrastructures();
}



function affiche_formulaire_infrastructures() {
	GLOBAL $frm;
	GLOBAL $id;
...
<tr>
	<td width="50%">
		...
		<?=$categorie_options?>
				...
				<tr>
					<td class="normal">Etat du infrastructures :</td>
					<td>
				...
				<tr>
					<td class="label" colspan=2><font color="red">Nom du infrastructures *</font>:
					<?php
				...
				<tr>
					<td class="entete" colspan="2">Photos des infrastructures :</td>
				</tr>
...
<?php
				...
				<tr valign=top>
					<td class="label" colspan="2">Description (affichage dans les fiches infrastructures) :<br /></td>
				</tr>
				<tr valign=top>
					<td class=normal colspan=2>
					<p>
...
function supprime_infrastructures($id) {
/* Supprime l infrastructures sp&eacute;cifici&eacute; par $id. Il faut supprimer l infrastructures
 * puis les entr&eacute;es correspondantes de la table infrastructures_categories. */

	/* Charge les infos du infrastructures. */
	$sql = query("
	...
	$prod = fetch_assoc($qid);
		...
		/* Efface l infrastructures */
	query("DELETE FROM infrastructures WHERE id = $id");

	/* Efface ce infrastructures de la table infrastructures_categories */
	
	$sqlCat = "SELECT categorie_id FROM infrastructures_categories WHERE infrastructures_id = $id";
	...
	if ($resCat->RowCount() > 0){
		...
		while ($cat = fetch_assoc($resCat)) {
			...
			or DIE('Une erreur de connexion &agrave; la base s est infrastructurese ' . __LINE__ . '.<p>' . MYSQL_ERROR());
		
		}
		
	}
	
	query("DELETE FROM infrastructures_categories WHERE infrastructures_id = $id");
	
	
	
	?>
		<p class="normal">
		L infrastructures <b><?echo html_entity_decode($prod['nom']); ?></b> a &eacute;t&eacute; effac&eacute;.
		</p>
	<?php
}

function supprime_fichier($id, $file) {
/* Supprime le infrastructures sp&eacute;cifici&eacute; par $id. Il faut supprimer le infrastructures
 * puis les entr&eacute;es correspondantes de la table infrastructures_categories. */
	global $the_path ;
	/* Charge les infos du infrastructuresx. */

	switch($file) {

	case "image1" :
	$sql = "SELECT image1 FROM infrastructures WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	query("UPDATE infrastructures SET image1 = '' WHERE id = '$id'");
	break;
	
	case "image2" :
	$sql = "SELECT image2 FROM infrastructures WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE infrastructures SET image2 = '' WHERE id = '$id'");
	break;
	
	case "image3" :
	$sql = "SELECT image3 FROM infrastructures WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE infrastructures SET image3 = '' WHERE id = '$id'");
	break;
	
	case "image4" :
	$sql = "SELECT image4 FROM infrastructures WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE infrastructures SET image4 = '' WHERE id = '$id'");
	break;
	
    case "image5" :
	$sql = "SELECT image5 FROM infrastructures WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE infrastructures SET image5 = '' WHERE id = '$id'");
	break;
	
	case "image6" :
	$sql = "SELECT image6 FROM infrastructures WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE infrastructures SET image6 = '' WHERE id = '$id'");
	break;
	
	case "image7" :
	$sql = "SELECT image7 FROM infrastructures WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE infrastructures SET image7 = '' WHERE id = '$id'");
	break;

    case "image8" :
	$sql = "SELECT image8 FROM infrastructures WHERE id = '$id'";
	$res = query($sql);
	$file =fetch_assoc($res);
	mysql_query("UPDATE infrastructures SET image8 = '' WHERE id = '$id'");
	break;
...
function insere_infrastructures($id, $img1, $img2, $img3, $img4, $img5, $frm) {
/* Ajoute un nouveau sous-infrastructures sous le parent $id.  Les champs sont dans la variable $frm */
	...
	$infrastructures = addSlashes($frm['infrastructures']);
	$infrastructure = addSlashes($frm['infrastructure']);
	$infrastructures = addSlashes($frm['infrastructures']);
	$type_infrastructures = addSlashes($frm['infrastructures']);
	$materiaux = addSlashes($frm['materiaux']);
	/*ajoute l infrastructures dans la table infrastructures */
	...
	, '$frm[comments]'
	,'".$frm['on_infrastructures']."'
	
	)
	";
	
	$qid =query($sqlProd) or DIE('Une erreur de connexion &agrave; la base s est infrastructurese ' . __LINE__ . '.<p>' . MYSQL_ERROR());
	
	/* r&eacute;cup&egrave;re l'id du infrastructures cr&eacute;&eacute; */
	$infrastructures_id = mysql_insert_id();

	/* ajoute le infrastructures sous les cat&eacute;gories sp&eacute;cifi&eacute;es */
	for ($i = 0; $i < count(vn($frm['categories'])); $i++) {
		$qid = query("
		INSERT INTO infrastructures_categories (categorie_id, infrastructures_id)
		VALUES ('{$frm['categories'][$i]}', '$infrastructures_id')
		");
...
function maj_infrastructures($id, $img1, $img2, $img3, $img4, $img5, $img6, $img7, $img8,  $frm) {
/* Met &agrave; jour le infrastructures $id avec de nouvelles valeurs. Les champs sont dans $frm */

	valide($frm['on_infrastructures']);
	
	/* Met &agrave; jour la table infrastructures */
	...
	$sql .= "
		...
		,etat = '$frm[etat]'
		,date_maj = now()
		,on_infrastructures= '$frm[oninfrastructures]'
		
		
	WHERE id = ".intval($id)."
	";
	
	$qid = query($sql) or DIE('Une erreur de connexion &agrave; la base s est infrastructurese ' . __LINE__ . '.<p>' . MYSQL_ERROR());
	...
	/* Efface toutes les cat&eacute;gories auxquelles le infrastructures est associ&eacute; */
	$qid = query("
	DELETE FROM infrastructures_categories
	WHERE infrastructures_id = $id
	");
	

	/* Ajoute les  associations pour toutes les cat&eacute;gories auxquelles ce infrastructures
...
function affiche_liste_infrastructures($start, $frm) 
{
global $wwwroot;
global $categorie_options;
global $repertoire_images;
...
<table border="0" cellpadding="0" cellspacing="2" width="100%">
    ...
    <tr>
      <td class="normal">Etat du infrastructures : <br />
	  <span class="normal">
...
<tr>
	<td class="entete" colspan="10">Liste des infrastructures</td>
</tr>
<tr><td colspan="10" class="normal">
<img src="images/add.png" width="16" height="16" alt="" border="0" align="middle"><a href="<?echo $wwwroot?>/gestions/infrastructures.php?mode=ajout">Ajouter un infrastructures</a>
</td></tr>
			...
			if (isset($frm['categorie'])) {
				...
				$Links-> SqlRequest = "SELECT p.id, p.nom, p.description, p.image1, p.etat, p.date_maj FROM infrastructures p, infrastructures_categories pc,categories c WHERE p.id = pc.infrastructures_id AND c.id = pc.categorie_id ".$where." ORDER BY parent_id ASC";	
				
			} else {
			
				$Links-> SqlRequest = "SELECT p.id, p.diametre, p.capacite, p.formes, p.type_paroi, p.materiaux,  p.nom_".$_SESSION['langue'].", p.description_".$_SESSION['langue'].", p.image1, p.pdf, p.etat, p.date_maj FROM infrastructures p ORDER BY id DESC";
			...
			echo "<tr><td class=\"normal\"><b>Aucun infrastructures enregistr&eacute; dans la base pour ce crit&egrave;re</b></td></tr>"; } 
			...
			?>
					<tr bgcolor="<?echo ($i % 2 == 0 ? '#F4F4F4' : '#ffffff' );?>">
						...
						<a onClick="Javascript:return confirm('&ecirc;tes-vous s&ucirc;r de vouloir supprimer le infrastructures <?=$ligne['nom_'.$_SESSION['langue'].''] ?> ?');" class=normal title="Supprimer <?=$ligne['nom_'.$_SESSION['langue'].''] ?>" href="<?=$_SERVER['PHP_SELF']?>?mode=suppr&id=<?=$ligne['id'] ?>">
...
?>
						...
						<td class=normal align="center"><a class=normal title="Modifier ce infrastructures" href="<?=$_SERVER['PHP_SELF']?>?mode=modif&id=<?=$ligne['id'] ?>"><?=html_entity_decode($ligne['nom_'.$_SESSION['langue'].''])?></a></td>
...
<?PHP
function valide_form_infrastructures(&$frm, &$erreurs) {
